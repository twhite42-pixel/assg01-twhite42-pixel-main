...
{
  // Validate addresses
  if (baseAddress < 0 || boundsAddress < 0 || baseAddress >= 1000 || boundsAddress >= 1000 || baseAddress > boundsAddress)
  {
    throw SimulatorException("Error: invalid memory initialization parameters");
  }

  // Reset first in case memory was already allocated
  this->reset();

  // Save new values
  this->memoryBaseAddress = baseAddress;
  this->memoryBoundsAddress = boundsAddress;
  this->memorySize = boundsAddress - baseAddress + 1;
